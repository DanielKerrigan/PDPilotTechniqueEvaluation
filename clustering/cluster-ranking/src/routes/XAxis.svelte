<script lang="ts">
	import { defaultFormat } from '$lib/vis-utils';
	import type { ScaleContinuousNumeric, ScaleBand, ScalePoint } from 'd3-scale';
	import Label from './Label.svelte';

	export let scale: ScaleContinuousNumeric<number, number> | ScaleBand<number> | ScalePoint<number>;
...
<g transform="translate({x},{y})">
	<g>
		{#if showBaseline}
			<line x1={scale.range()[0]} x2={scale.range()[1]} stroke={baselineColor} />
		{/if}
